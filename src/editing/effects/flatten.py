...
def _build_flatten_guidance(
    ...
    """
    Construct the guidance field for texture flattening
    Keep the original gradient only if there is an edge between p and q.
    """
...
def texture_flattening(
    ...
    """
    Flattens the texture of the masked region while preserving sharp edges.
    Uses the Canny edge detector to identify edges in the masked region.
    ...
    # Standard formula for converting RGB to luminance (Y channel)
    luminance = (
