...
def _build_mixed_guidance(
    ...
    """
    Build the guidance field for mixed seamless cloning (Eq. 13):
    Retain the stronger gradient between source (g) and destination (f*).
    """
    ...
    for dy, dx in _NEIGHBORS_4:
        ...
        if y1 > y0 and x1 > x0:
            v_src = (
                source_channel[y0:y1, x0:x1] - source_channel[y0 + dy : y1 + dy, x0 + dx : x1 + dx]
            )
            v_dst = (
                dest_channel[y0:y1, x0:x1] - dest_channel[y0 + dy : y1 + dy, x0 + dx : x1 + dx]
            )
...
def mixed_gradients(
    ...
    """
    Mixed Seamless cloning for RGB images.
    Preserves strong textures from the destination image (e.g. brick walls)
    while blending the source image.
    """
